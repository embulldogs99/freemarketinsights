function metastats(){
  ...
  xhttp.onreadystatechange=function(){
    if (this.readyState==4 && this.status==200){
      ...
      metastats='';
      metastats+='In our most recent data pull on '+response.DateRun+', we surveyed '+response.StockCount+' different analyst target prices from before '+response.PullDate;
      metastats+='.  Of those predictions, '+(response.PercentBull*100).toFixed(2)+'% where bullish and '+(response.PercentBear*100).toFixed(2)+'% where bearish.<br><br> Of those predictions, ';
      metastats+=parseFloat(response.PercentCall*100).toFixed(2)+' predicted the future stock price direction correctly. One average the actual stock return was '+parseFloat(response.AvgReturn*100).toFixed(2)+'% while the average expected return was '+parseFloat(response.AvgExpectedReturn).toFixed(2)+'%. On average, the analysts missed their prediction by '+parseFloat(response.AvgReturnDelta*100).toFixed(2);
