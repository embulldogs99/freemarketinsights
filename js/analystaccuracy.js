function metastats(){
  ...
  xhttp.onreadystatechange=function(){
    if (this.readyState==4 && this.status==200){
      ...
      metastats+=(response.PercentCall*100).toFixed(2)+' predicted the future stock price direction correctly. One average the actual stock return was '+(response.AvgReturn*100).toFixed(2)+'% while the average expected return was '+(response.AvgExpectedReturn).toFixed(2)+'%. On average, the analysts missed their prediction by '+(response.AvgReturnDelta*100).toFixed(2);
